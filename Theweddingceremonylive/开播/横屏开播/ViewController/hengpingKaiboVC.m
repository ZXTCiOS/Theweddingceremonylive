...
@interface hengpingKaiboVC ()<NIMNetCallManagerDelegate, NIMChatroomManagerDelegate, NIMChatManagerDelegate,  UITableViewDelegate, UITableViewDataSource, UICollectionViewDelegate, UICollectionViewDataSource>

@property (nonatomic, strong) UIView *localPreView;
@property (nonatomic, strong) hengpingMaskView *maskview;
...
- (void)configMaskview{
    self.maskview = [[NSBundle mainBundle] loadNibNamed:NSStringFromClass([hengpingMaskView class]) owner:nil options:nil].firstObject;
...
- (void)audienceListPerMin{
    self.timer = [NSTimer scheduledTimerWithTimeInterval:30 block:^(NSTimer * _Nonnull timer) {
        ...
        NSDictionary *para = @{@"uid": uid, @"token": token, @"renshu": count, @"ordersn": self.orderID, @"direction": @"0"};
