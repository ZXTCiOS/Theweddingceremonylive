...
@interface hengpingWatchVC ()<UITableViewDelegate, UITableViewDataSource, UICollectionViewDelegate, UICollectionViewDataSource, NIMChatroomManagerDelegate, NIMChatManagerDelegate, NIMNetCallManagerDelegate, NIMSystemNotificationManagerDelegate>

//@property (nonatomic, strong) UIScrollView *scrollV;                // 遮罩层
@property (nonatomic, strong) hengpingWatchMaskView *maskview;
...
- (void)viewDidLoad {
    ...
    //self.roomid = [userDefault objectForKey:user_nickname];
    //self.roomName = @"xixi";
...
- (void)configBottomBtn:(hengpingWatchMaskView *)view{
    view.first = ^(){//聊天
    ...
    view.third = ^(){//礼物
        [UIView animateWithDuration:0.5 delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
            self.giftV.frame = CGRectMake(0, kScreenH- 75, kScreenW, 75);
        } completion:nil];
    };
    view.fourth = ^(){//分享
        [UIView animateWithDuration:0.25 animations:^{
...
- (NIMNetCallVideoCaptureParam *)para{
    ...
    para.videoCaptureOrientation = NIMVideoOrientationLandscapeRight;
    return para;
}
...
- (hengpingWatchMaskView *)maskview{
    if (!_maskview) {
        _maskview = [[NSBundle mainBundle] loadNibNamed:NSStringFromClass([hengpingWatchMaskView class]) owner:nil options:nil].firstObject;
        _maskview.frame = CGRectMake(0, 0, kScreenW, kScreenH);
...
- (GiftView *)giftV{
    if (!_giftV) {
        _giftV = [[GiftView alloc] initWithDirection:screenDirectionH];
        ...
        [_giftV.send bk_addEventHandler:^(id sender) {
            ...
            [DNNetworking postWithURLString:post_sendgift parameters:para success:^(id obj) {
                ...
                if ([code isEqualToString:@"1000"]) {
                    ...
                    if (!from) {
                        from = [userDefault objectForKey:user_nickname];
                    }
...
- (SendRedBagView *)redBag{
    if (!_redBag) {
        ...
        _redBag.frame = CGRectMake((kScreenW - kScreenH + 60)/2, 20, kScreenH- 60, 360);
